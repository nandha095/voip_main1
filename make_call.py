...
RING_TIMEOUT_SECONDS = int(os.getenv("RING_TIMEOUT_SECONDS", "120"))
...
async def make_call(destination_number: str, status_callback=None) -> subprocess.Popen[bytes]:
    """
...
async def _monitor_call_output(
    ...
    try:
        while True:
            line_bytes = await asyncio.to_thread(process.stdout.readline)
            if not line_bytes:
                break
...
async def hangup_call(process: subprocess.Popen[bytes]) -> bool:
    """
    ...
    try:
        try:
            if process.stdin:
                await asyncio.to_thread(process.stdin.write, b"h\n")
